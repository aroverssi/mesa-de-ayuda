...
// Función para mostrar los tickets con filtros avanzados
function mostrarTickets(isAdmin) {
    ...
    const fechaInicio = new Date(document.getElementById(isAdmin ? "adminStartDate" : "userStartDate")?.value || "1970-01-01");
    const fechaFin = new Date(document.getElementById(isAdmin ? "adminEndDate" : "userEndDate")?.value || "2100-12-31");
    ...
    onSnapshot(consulta, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            row.innerHTML = isAdmin
                ? `<td>${ticket.consecutivo}</td><td>${ticket.usuario}</td><td>${ticket.company}</td><td>${ticket.descripcion}</td><td>${ticket.estado}</td>`
                : `<td>${ticket.consecutivo}</td><td>${ticket.usuario}</td><td>${ticket.company}</td><td>${ticket.descripcion}</td>`;
    ...
    onSnapshot(collection(db, "tickets"), (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            if (ticket.estado === "cerrado") {
                totalCerrados++;
                const tiempoResolucion = (ticket.fechaCierre?.seconds - ticket.fechaApertura?.seconds) / 3600 || 0;
        ...
        statsList.innerHTML = `
            ...
            <li>Promedio de Resolución: ${promedioResolucion} horas</li>`;
    });
}
