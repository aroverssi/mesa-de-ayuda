...
function mostrarTickets(isAdmin) {
    ...
    const filtroUsuario = document.getElementById(isAdmin ? "adminFilterUser" : "userFilterUser").value || "";
    const filtroCompany = document.getElementById(isAdmin ? "adminFilterCompany" : "userFilterCompany").value || "";
    const fechaInicio = document.getElementById(isAdmin ? "adminFilterStartDate" : "userFilterStartDate").value || "";
    const fechaFin = document.getElementById(isAdmin ? "adminFilterEndDate" : "userFilterEndDate").value || "";

    let consulta = collection(db, "tickets");
    const filtros = [];

    if (filtroUsuario) filtros.push(where("usuario", "==", filtroUsuario));
    if (filtroCompany) filtros.push(where("company", "==", filtroCompany));
    if (fechaInicio) filtros.push(where("fechaApertura", ">=", Timestamp.fromDate(new Date(fechaInicio))));
    if (fechaFin) filtros.push(where("fechaApertura", "<=", Timestamp.fromDate(new Date(fechaFin))));
